...
public class ToolApprovalRequest {
    ...
    /**
     * 会话唯一标识 (Conversation ID)，用于定位智能体运行时上下文及消息记录
     */
    private final Long cid;

    /**
     * 待审批工具调用的唯一标识 (Tool Call ID)，对应工具执行等待屏障及工具消息关联键
     */
    private final String toolCallId;

    /**
     * 人工审批决策（如 "ALLOW" 放行、"DENY" 拒绝，大小写不敏感）
     */
    private final String decision;

    /**
     * 是否勾选“总是放行/信任此规则”。若为 true 且 decision 为 ALLOW，将触发授信规则写回工作区配置
     */
    private final boolean alwaysAllow;

    /**
     * 待审批的工具名称（如 "run_command" 等），用于在总是放行时关联授信规则
     */
    private final String toolName;

    /**
     * 规则匹配的内容模式/正则匹配式（可选，配合 alwaysAllow 写入自动化放行规则，如命令或路径模式）
     */
    private final String contentPattern;

    /**
     * 用户人工修改后的自定义入参覆盖 JSON 字符串（可选，若非空则优先替代大模型生成的原始入参执行）
     */
    private final String customArgOverride;
}
