...
public class McpClientInstance {
    ...
    public synchronized void start() throws Exception {
        ...
        // 执行握手交互
        try {
            // 快速失败探活：stdio 模式下，若子进程在短暂观察窗内已退出（如主类不存在、启动参数非法），
            // 则无需干等完整握手超时，直接判定启动失败并返回。
            // waitFor 返回 true 即代表进程已退出；返回 false 说明进程仍存活，放行进入正常握手
            if (!sse && process != null && process.waitFor(500, TimeUnit.MILLISECONDS)) {
                int code = process.exitValue();
                status = "OFFLINE";
                shutdown();
                throw new IOException("MCP 子进程提前退出 (exitCode=" + code + ")，请检查启动命令与 classpath 配置");
            }
            initAndHandshake();
            status = "RUNNING";
        } catch (Exception e) {
    ...
    public void refreshTools() throws Exception {
        ...
        if (result != null && result.containsKey("tools")) {
            ...
            for (int i = 0; i < toolsArr.size(); i++) {
                ...
                // 采用 mcp__{serverName}__{toolName} 域隔离命名规范，杜绝与宿主及其他 MCP 工具命名冲突
                list.add(new McpCuteTool(name, tName, tDesc, schemaStr, this));
            }
        }
