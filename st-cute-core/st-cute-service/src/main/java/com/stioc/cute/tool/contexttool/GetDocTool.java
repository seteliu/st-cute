...
public class GetDocTool implements CuteTool {
    ...
    public String execute(Map<String, Object> arguments, ToolExecutionContext context) {
        GetDocArgs docArgs = GetDocArgs.from(arguments);
        String topic = docArgs.topic();
        if (topic == null || topic.isBlank()) {
            return ToolResult.error("参数 'topic' 不能为空。可选值：convention, rule, skill, mcp, hook, all");
        }
        ...
        if ("all".equals(topic)) {
            ...
            if (sb.isEmpty()) {
                return ToolResult.error("未能加载任何文档");
            }
            return sb.toString();
        }
        ...
        if (docPath == null) {
            return ToolResult.error("未知的文档主题: " + topic + "。可选值：convention, rule, skill, mcp, hook, all");
        }

        String content = readClasspathFile(docPath);
        if (content == null) {
            return ToolResult.error("文档文件不存在或读取失败: " + docPath);
        }
