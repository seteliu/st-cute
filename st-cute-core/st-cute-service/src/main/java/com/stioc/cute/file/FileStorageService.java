 ...
 * 多形态路径统一解析 {@link #resolveFlexiblePath(String, String)}、附件沙箱解析
 * {@link #resolveAttachmentFile(String)}、缩略图读取、文件与会话附件级联删除，
 * 另附文件扩展名 / MIME 推导静态工具。
 * </p>
 * <p>
 * 路径语义：上传返回附件文件的绝对路径；读取侧统一接受项目相对路径或绝对路径。
 * HTTP 资源访问接口走 {@link #resolveAttachmentFile(String)}，仅限附件沙箱内的文件。
 * </p>
 */
@Slf4j
@Service
public class FileStorageService {
    ...
    public File getFilesRootDir() {
        return new File(ContractFile.getGlobalDir(), "files");
    }
    ...
    /**
     * 附件沙箱路径解析：仅允许读取 {@code ~/.st-cute/files} 目录内的真实文件。
     * <p>
     * 与 {@link #resolveFlexiblePath(String, String)} 的区别：后者面向 Agent 工具链，
     * 需保留读取项目工作区任意文件的能力；本方法专供 HTTP 资源接口（文件预览/下载）使用，
     * 严格收敛到附件沙箱，防止任意文件读取。
     *
     * @param pathVal 目标路径（相对路径时以附件根目录为基准解析）
     * @return 沙箱内且真实存在的常规文件；越权、不存在或为目录时返回 null
     */
    public File resolveAttachmentFile(String pathVal) {
        if (!StringUtils.hasText(pathVal)) {
            return null;
        }
        try {
            File rootDir = getFilesRootDir();
            Path rootPath = rootDir.getAbsoluteFile().toPath().normalize();
            Path targetPath = Paths.get(pathVal.trim().replace('\\', '/'));
            if (!targetPath.isAbsolute()) {
                targetPath = rootPath.resolve(targetPath);
            }
            // 先规范化消解 .. 后再比较，避免 ../ 穿越出沙箱
            Path normalized = targetPath.toAbsolutePath().normalize();
            if (!normalized.startsWith(rootPath)) {
                log.warn("附件访问越权被拦截: path={}", pathVal);
                return null;
            }
            // toRealPath 消解符号链接，防止软链指向沙箱外文件
            Path realPath = normalized.toRealPath();
            if (!realPath.startsWith(rootPath)) {
                log.warn("附件访问越权被拦截（符号链接逃逸）: path={}", pathVal);
                return null;
            }
            File file = realPath.toFile();
            return file.isFile() ? file : null;
        } catch (Exception e) {
            // 路径非法、文件不存在等一律按未找到处理，不向调用方暴露细节
            log.warn("附件沙箱路径解析失败: path={}, error={}", pathVal, e.getMessage());
            return null;
        }
    }
