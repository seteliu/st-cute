...
* **全局级 Hook 配置**：`~/.st-cute/hooks.json`
* **项目级 Hook 配置**：`{projectBasePath}/.st-cute/hooks.json`（优先级最高）
* **项目通用级 Hook 配置**：`{projectBasePath}/.agents/hooks.json`（项目共享，适合提交 Git 团队统一）

Hook 为叠加型契约，三层规则全部生效；装载注入顺序：项目级 → 项目通用级 → 全局级（优先级高者注入在列表前面），同名规则互不冲突（项目规则以项目路径为命名空间）。
...
后端 `HookService` 在执行 Hook 脚本时具备以下特性：
