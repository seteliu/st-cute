...
* **全局级技能目录**：`~/.st-cute/skills/`
* **项目级技能目录**：`{projectBasePath}/.st-cute/skills/`（同名覆盖优先级最高）
* **项目通用级技能目录**：`{projectBasePath}/.agents/skills/`（项目共享，适合提交 Git 团队统一）
...
1. **热扫描**（覆盖型契约）：后端 `SkillManagerServiceImpl` 会自动扫描全局级与项目两级目录，装载顺序 全局级 → 项目通用级 → 项目级，同名技能后写覆盖，项目级最终生效。
