...
* **全局级配置**：`~/.st-cute/mcp_servers.json`
* **项目级配置**：`{projectBasePath}/.st-cute/mcp_servers.json`（同名覆盖优先级最高）
* **项目通用级配置**：`{projectBasePath}/.agents/mcp_servers.json`（项目共享，适合提交 Git 团队统一）
...
* **配置合并策略**（覆盖型契约）：会话装配时按 全局级 → 项目通用级 → 项目级 顺序读取合并 `mcp_servers.json`，同名服务后写覆盖，项目级最终生效。
