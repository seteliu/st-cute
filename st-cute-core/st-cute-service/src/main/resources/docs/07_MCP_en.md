...
* **Global Configuration**: `~/.st-cute/mcp_servers.json`
* **Project-level Configuration**: `{projectBasePath}/.st-cute/mcp_servers.json` (highest override priority)
* **Project Common Configuration**: `{projectBasePath}/.agents/mcp_servers.json` (project-shared, suitable for Git versioning)
...
* **Config Merge Strategy** (Overriding Contract): At session assembly, `mcp_servers.json` configs are read and merged in the order Global → Project Common → Project-level; same-named servers are overridden by later reads, so the project-level config takes final effect.
