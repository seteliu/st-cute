...
* **Project-level Hook Configuration**: `{projectBasePath}/.st-cute/hooks.json` (highest priority)
* **Project Common Hook Configuration**: `{projectBasePath}/.agents/hooks.json` (project-shared, suitable for Git versioning)

Hook is a stacking contract — rules from all three levels take effect; loading/injection order: Project-level → Project Common → Global (higher priority is placed first in the rule list). Same-named rules do not conflict (project rules are namespaced by project path).
...
Backend `HookService` provides the following features during Hook execution:
...
| **`permissionMode`** | String | Current permission mode (e.g. `STRICT_APPROVAL` / `RELAXED_APPROVAL`) |
