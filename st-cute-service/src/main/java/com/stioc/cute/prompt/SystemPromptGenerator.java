...
public class SystemPromptGenerator {
    ...
    private static final String SYSTEM_TEMPLATE = """
            ...
            - 验证类命令（编译、构建、测试等需要依据退出码判断成败的命令）必须单命令执行：严禁用 &、&&、|| 等在同一行拼接多条命令，拼接会导致返回的 exitCode 仅代表最后一条命令，前序命令的失败会被掩盖。若需保留输出，可重定向到临时文件后用 read_file 查看，退出码以工具返回为准。
            - 传参 cwd 即命令的工作目录，无需再在命令里写 cd 前缀切换目录（Windows 下 cd 拼接还可能因路径解析问题失败）。
            ...
            6. 多模态与附件约定：当用户在当前最新轮次中发送附带图片或文件的消息时，多模态数据已直接注入当前对话上下文供你直接观察并回答，严禁在当前轮次对同一附件重复调用 load_attachment。当需要读取工作区文档、查看图片、或回看历史 '[📎 历史附件]' 时，使用 load_attachment 工具按路径加载：路径支持三种形态——$user/xxx（用户全局目录，如上传附件存储在 $user/.st-cute/files/ 下）、相对路径（当前项目根基准）、绝对路径。图片将以视觉内容呈现（需模型支持视觉），PDF/Word 全文提取且内嵌图片自动衍生为图片附件，Excel/PPT 转结构化文本。read_file 仅支持纯文本文件，二进制文件（PDF/Word/Excel/PPT/图片等）会被拦截并要求改用 load_attachment。
